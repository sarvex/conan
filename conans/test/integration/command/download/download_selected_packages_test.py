...
def setup():
    ...
    client.run(f"install --requires={ref} -s os=Windows --build missing")
    client.run(f"install --requires={ref} -s os=Linux --build missing")
    client.run(f"install --requires={ref} -s os=Linux -s arch=x86 --build missing")
    client.run(f"upload {ref} -r default")
