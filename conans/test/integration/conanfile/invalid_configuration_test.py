...
class MyPkg(ConanFile):
    ...
    def test_install_method(self):
        self.client.run(f"install . {self.settings_msvc15}")

        error = self.client.run(f"install . {self.settings_msvc12}", assert_error=True)
        self.assertEqual(error, ERROR_INVALID_CONFIGURATION)
        self.assertIn("Invalid configuration: user says that compiler.version=12 is invalid",
                      self.client.out)

    def test_info_method(self):
        self.client.run(f"graph info . {self.settings_msvc15}")

        error = self.client.run(
            f"graph info . {self.settings_msvc12}", assert_error=True
        )
        self.assertEqual(error, ERROR_INVALID_CONFIGURATION)
    ...
    def test_create_method(self):
        self.client.run(
            f"create . --name=name --version=ver --user=jgsogo --channel=test {self.settings_msvc15}"
        )

        error = self.client.run(
            f"create . --name=name --version=ver --user=jgsogo --channel=test {self.settings_msvc12}",
            assert_error=True,
        )
        self.assertEqual(error, ERROR_INVALID_CONFIGURATION)
    ...
    def test_as_requirement(self):
        self.client.run(f"create . --name=name --version=ver {self.settings_msvc15}")
...
class MyPkg(ConanFile):
    ...
    """})
        self.client.run(
            f"create other/ --name=other --version=1.0 {self.settings_msvc15}"
        )

        error = self.client.run(
            f"create other/ --name=other --version=1.0 {self.settings_msvc12}",
            assert_error=True,
        )
        self.assertEqual(error, ERROR_INVALID_CONFIGURATION)
